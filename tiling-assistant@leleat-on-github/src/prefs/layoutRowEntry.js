...
export const LayoutRowEntry = GObject.registerClass(
    {
        ...
        InternalChildren: ['rectEntry', 'rectLabel', 'rectAppButton'],
    ...
    class TilingLayoutRowEntry extends Gtk.Box {
        _init(idx, item) {
            ...
            this._rectLabel.set_label(
                `<span size="x-small">Rect ${idx}</span>`,
            );
            const loop = item.loopType ? `--${item.loopType}` : '';
            const rect = item.rect;
            const text =
                Object.keys(rect).length !== 0 ?
            ...
            if (!text) {
                if (idx === 0) {
                    ...
                    this._rectEntry.set_placeholder_text(
                        'x--y--width--height[--h|v]',
                    );
                }
            }

...
const {Layout, LayoutItem} = Me.imports.src.common;
 ...
 * A LayoutItem has a { rect, appId, loopType }. The rect is mandatory, the rest not.
 */
...
var GuiRow = GObject.registerClass({
	Signals: { "changed": { param_types: [GObject.TYPE_BOOLEAN] } },
}, class PopupLayoutRow extends Gtk.ListBoxRow {

	// use static variable to make sure the indeces are unique since just using
	...
	_init(layout) {
		...
		// layout is just a simple parsed object from the json file or null,
		// if the row is empty. Convert it into (empty) Layout object.
		...
		this._nameEntry = new Gtk.Entry({
			// for some reason the text and placeholder are both shown when
			// saving so only set placeholder, if there is no name...
			placeholder_text: this._layout.getName() === "" ? _("Nameless Layout...") : ""
		});
		this._nameEntry.connect("changed", this._onNameEntryChanged.bind(this));
		topBox.append(this._nameEntry);

		/***********************************************
		 * middle row: rect entries and layout preview *
		 ***********************************************/
		...
		mainFrame.set_label(`    Layout ${PopupLayoutRow.instanceCount}    `);
	...
	getLayout() {
		...
		while (this._layout.getItemCount() > 0 && !ok) {
			this._layout.removeItem(idx);
			[ok, , idx] = this._layout.validate()
		}

		return this._layout.getItemCount() > 0 ? this._layout : null;
	};
...
const GuiEntry = GObject.registerClass({
	...
	// @item is a LayoutItem but may be an "empty" LayoutItem.
	// With empty I mean, it has no valid rect defined.
	_init(idx, item) {
		super._init({
			orientation: Gtk.Orientation.HORIZONTAL,
			spacing: 8
		...
		const entry = new Gtk.Entry({
			...
			placeholder_text: idx === 0 ? _("This tooltip for help...") : "",
			hexpand: true
		});
	...
	// Proper format: "Float--Float--Float--Float[--String]".
	// For now, the errors aren't communicted to the user, because
	// it may be annoying, if they get notified after every key press

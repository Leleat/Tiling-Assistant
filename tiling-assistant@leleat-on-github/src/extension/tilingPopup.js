...
export const TilingSwitcherPopup = GObject.registerClass(
    ...
    class TilingSwitcherPopup extends AltTab.TilingAppSwitcherPopup {
        ...
        vfunc_allocate(box) {
            ...
            const rightPadding = this.get_theme_node().get_padding(
                St.Side.RIGHT,
            );
            const hPadding = leftPadding + rightPadding;

            const [, childNaturalHeight] =
            ...
            childBox.x1 = Math.max(
                freeScreenRect.x + leftPadding,
                freeScreenRect.x +
                    Math.floor((freeScreenRect.width - childNaturalWidth) / 2),
            );
            childBox.x2 = Math.min(
                freeScreenRect.x2 - rightPadding,
                childBox.x1 + childNaturalWidth,
            );
            childBox.y1 =
                freeScreenRect.y +
                Math.floor((freeScreenRect.height - childNaturalHeight) / 2);
            ...
            if (this._thumbnails) {
                ...
                const leftPadd = this.get_theme_node().get_padding(
                    St.Side.LEFT,
                );
                const rightPadd = this.get_theme_node().get_padding(
                    St.Side.RIGHT,
                );
                const bottomPadding = this.get_theme_node().get_padding(
                    St.Side.BOTTOM,
                );
                ...
                const [, cNatHeight] =
                    this._thumbnails.get_preferred_height(-1);
        ...
        _finish(timestamp) {
            ...
            const window =
                appIcon.cachedWindows[Math.max(0, this._currentWindow)];
            this._tileWindow(window);
        ...
        _tileWindow(window) {
            ...
            const isShiftPressed = Util.isModPressed(
                Clutter.ModifierType.SHIFT_MASK,
            );
            const isAltPressed = Util.isModPressed(
...
const TSwitcherList = GObject.registerClass(
    class TilingSwitcherList extends AltTab.TilingAppSwitcher {
        _setIconSize() {
            ...
            const parentPadding = this.get_parent()
                .get_theme_node()
                .get_horizontal_padding();
            const availWidth =
                freeScreenRect.width -
                parentPadding -

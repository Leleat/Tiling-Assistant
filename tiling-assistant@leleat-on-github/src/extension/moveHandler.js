...
export default class TilingMoveHandler {
        ...
        if (window.tiledRect || this._wasMaximizedOnStart) {
            ...
            if (Settings.getBoolean('low-performance-move-mode')) {
                this._movingTimerId = GLib.timeout_add(
                    ...
                    this._onMoving.bind(this, grabOp, window, true),
                );
                ...
                this._posChangedId = window.connect(
                    'position-changed',
                    this._onMoving.bind(this, grabOp, window, false),
                );
    ...
    _onMoveFinished(window) {
        try {
            ...
            if (this._tileRect) {
                ...
                const pointerPos = {
                    x: global.get_pointer()[0],
                    y: global.get_pointer()[1],
                };
                const twHovered = topTileGroup.some((w) =>
                    w.tiledRect.containsPoint(pointerPos),
                );
                if (
                    this._currPreviewMode === MoveModes.ADAPTIVE_TILING &&
                    !this._splitRects.size &&
                    twHovered
                ) {
                ...
                this._splitRects.forEach((rect, w) =>
                    Twm.tile(w, rect, {openTilingPopup: false}),
                );
    ...
    _onMoving(grabOp, window, lowPerfMode = false) {
        ...
        if (lowPerfMode) {
            ...
            const movementDist = Util.getDistance(
        ...
        const useAdaptiveTiling =
            (defaultMode !== MoveModes.ADAPTIVE_TILING &&
                pressed[adaptiveMod]) ||
            (noMod && defaultMode === MoveModes.ADAPTIVE_TILING);
        const usefavLayout =
            (defaultMode !== MoveModes.FAVORITE_LAYOUT && pressed[favMod]) ||
            (noMod && defaultMode === MoveModes.FAVORITE_LAYOUT);
        const useIgnoreTa =
            (defaultMode !== MoveModes.IGNORE_TA && pressed[ignoreTAMod]) ||
    ...
    _edgeTilingPreview(window, grabOp) {
        ...
        const useGracePeriod = Settings.getBoolean(
        ...
        const workArea = new Rect(
            window.get_work_area_for_monitor(this._monitorNr),
        );

        const vDetectionSize = Settings.getInt('vertical-preview-area');
        const pointerAtTopEdge =
            this._lastPointerPos.y <= workArea.y + vDetectionSize;
        const pointerAtBottomEdge =
            this._lastPointerPos.y >= workArea.y2 - vDetectionSize;
        const hDetectionSize = Settings.getInt('horizontal-preview-area');
        const pointerAtLeftEdge =
            this._lastPointerPos.x <= workArea.x + hDetectionSize;
        const pointerAtRightEdge =
            this._lastPointerPos.x >= workArea.x2 - hDetectionSize;
        ...
        const tileTopLeftQuarter =
            pointerAtLeftEdge && (pointerAtTopEdge || windowAtTopEdge);
        const tileTopRightQuarter =
            pointerAtRightEdge && (pointerAtTopEdge || windowAtTopEdge);
        const tileBottomLeftQuarter =
            pointerAtLeftEdge && (pointerAtBottomEdge || windowAtBottomEdge);
        const tileBottomRightQuarter =
            pointerAtRightEdge && (pointerAtBottomEdge || windowAtBottomEdge);

        if (tileTopLeftQuarter) {
            this._tileRect = Twm.getTileFor(
    ...
    _adaptiveTilingPreview(window, grabOp) {
        ...
        const hoveredRect = screenRects.find((r) =>
            r.containsPoint(this._lastPointerPos),
        );
        if (!hoveredRect) {
            this._tilePreview.close();
            this._tileRect = null;
        ...
        const isSuperPressed = Util.isModPressed(
        ...
        if (isSuperPressed) {
            ...
            const edgeRadius = 50;
            const atTopEdge =
                this._lastPointerPos.y < hoveredRect.y + edgeRadius;
            const atBottomEdge =
                this._lastPointerPos.y > hoveredRect.y2 - edgeRadius;
            const atLeftEdge =
                this._lastPointerPos.x < hoveredRect.x + edgeRadius;
            const atRightEdge =
    ...
    _adaptiveTilingPreviewSingle(window, hoveredRect) {
        const atTop =
            this._lastPointerPos.y < hoveredRect.y + hoveredRect.height * 0.25;
        const atBottom =
            this._lastPointerPos.y > hoveredRect.y + hoveredRect.height * 0.75;
        const atRight =
            this._lastPointerPos.x > hoveredRect.x + hoveredRect.width * 0.75;
        const atLeft =
            this._lastPointerPos.x < hoveredRect.x + hoveredRect.width * 0.25;
        ...
        if (splitHorizontally || splitVertically) {
            ...
            const size =
                splitHorizontally ? hoveredRect.width : hoveredRect.height;
            const orientation =
                splitHorizontally ? Orientation.V : Orientation.H;
    ...
    _adaptiveTilingPreviewGroup(window, hoveredRect, hovered) {
        ...
        const factor =
            (
                (hovered.atLeftEdge && hoveredRect.x === workArea.x) ||
        ...
        if (hovered.atTopEdge || hovered.atBottomEdge) {
            const getX1X2 = (alignsAt) => {
                return this._topTileGroup.reduce(
                    (x1x2, w) => {
                        const currX = x1x2[0];
                        const currX2 = x1x2[1];
                        return alignsAt(w) ?
                                [
                                    Math.min(w.tiledRect.x, currX),
                                    Math.max(w.tiledRect.x2, currX2),
                                ]
                            :   x1x2;
                    },
            ...
            const alignTopEdge = (w) => {
                return (
                    hoveredRect.y === w.tiledRect.y ||
                    hoveredRect.y === w.tiledRect.y2
                );
            };
            const alignBottomEdge = (w) => {
                return (
                    hoveredRect.y2 === w.tiledRect.y2 ||
                    hoveredRect.y2 === w.tiledRect.y
                );
            };

            const [x1, x2] = getX1X2(
                hovered.atTopEdge ? alignTopEdge : alignBottomEdge,
            );
            ...
            const getY1Y2 = (alignsAt) => {
                return this._topTileGroup.reduce(
                    (y1y2, w) => {
                        ...
                        return alignsAt(w) ?
                                [
                                    Math.min(w.tiledRect.y, currY),
                                    Math.max(w.tiledRect.y2, currY2),
                                ]
                            :   y1y2;
                    },
            ...
            const alignLeftEdge = (w) => {
                return (
                    hoveredRect.x === w.tiledRect.x ||
                    hoveredRect.x === w.tiledRect.x2
                );
            };
            const alignRightEdge = (w) => {
                return (
                    hoveredRect.x2 === w.tiledRect.x2 ||
                    hoveredRect.x2 === w.tiledRect.x
                );
            };

            const [y1, y2] = getY1Y2(
                hovered.atLeftEdge ? alignLeftEdge : alignRightEdge,
            );
    ...
    class TilePreview extends WindowManager.TilePreview {
        ...
        open(window, tileRect, monitorIndex, animateTo = undefined) {
            ...
            if (!animateTo) {
                ...
                animateTo.mode =
                    animateTo.mode ?? Clutter.AnimationMode.EASE_OUT_QUAD;
            }

    ...
    class TileEditingMode extends St.Widget {
        ...
        open() {
            ...
            if (!openWindows.length || !this._windows.length) {
                ...
                const msg = _(
                    "Can't enter 'Tile Editing Mode', if no tiled window is visible.",
                );
            ...
            this._selectIndicator = new Indicator(
                window.tiledRect,
                this.monitor,
                params,
            );
    ...
    class TileEditingModeIndicator extends St.Widget {
        ...
        focus(rect, window = null) {
            ...
            const workArea = new Rect(
                activeWs.get_work_area_for_monitor(this._monitor),
            );
...
const DefaultKeyHandler = class DefaultKeyHandler {
    ...
    async handleKeyPress(keyEvent) {
        ...
        if (dir) {
            ...
            const topHalf = fullRect.getUnitAt(
                0,
                fullRect.height / 2,
                Orientation.H,
            );
            const rightHalf = fullRect.getUnitAt(
                1,
                fullRect.width / 2,
                Orientation.V,
            );
            const bottomHalf = fullRect.getUnitAt(
                1,
                fullRect.height / 2,
                Orientation.H,
            );
            const leftHalf = fullRect.getUnitAt(
                0,
                fullRect.width / 2,
                Orientation.V,
            );
            ...
            const openWindows = Twm.getWindows(allWs).filter(
                (w) => !this._windows.includes(w),
            );
    ...
    _focusInDir(dir) {
        const activeWs = global.workspace_manager.get_active_workspace();
        const workArea = new Rect(
            activeWs.get_work_area_for_monitor(this._tileEditor.monitor),
        );
        const tiledRects = this._windows.map((w) => w.tiledRect);
        const screenRects = tiledRects.concat(workArea.minus(tiledRects));
        const nearestRect = this._selectIndicator.rect.getNeighbor(
            dir,
            screenRects,
        );
        if (!nearestRect) {
        ...
        this._selectIndicator.focus(
            newWindow?.tiledRect ?? nearestRect,
            newWindow,
        );
    }
...
const SwapKeyHandler = class SwapKeyHandler extends DefaultKeyHandler {
    constructor(tileEditor) {
        ...
        const color = this._selectIndicator
            .get_theme_node()
        ...
        this._anchorIndicator.focus(
            this._selectIndicator.rect,
            this._selectIndicator.window,
        );
...
const MoveKeyHandler = class MoveKeyHandler extends DefaultKeyHandler {
    handleKeyPress(keyEvent) {
        const direction = Util.getDirection(keyEvent.get_key_symbol());
        const moveWorkspace =
            keyEvent.get_state() & Clutter.ModifierType.MOD1_MASK;
        ...
        if (direction) {
            ...
            if (moveWorkspace) {
                ...
                const newMonitor = global.display.get_monitor_neighbor_index(
                    currMonitor,
                    metaDir,
                );
                if (newMonitor === -1) {
...
const ResizeSideIndicator = GObject.registerClass(
    class ResizeSideIndicator extends St.Widget {
        _init(edge, activeRect) {
            const [width, height] =
                [Direction.N, Direction.S].includes(edge) ?
                    [200, 20]
                :   [20, 200];

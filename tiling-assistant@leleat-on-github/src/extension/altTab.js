...
import {AltTab, Extension, Main, SwitcherPopup} from '../dependencies/shell.js';
import {
    baseIconSizes,
...
export const TilingAppSwitcher = GObject.registerClass(
    class TilingAppSwitcher extends SwitcherPopup.SwitcherList {
                ...
                groupedWindows = windows.reduce((allGroups, w) => {
                    for (const group of allGroups) {
                        if (w.isTiled && Twm.getTileGroupFor(w).length > 1) {
                            ...
                            (!group[0].isTiled ||
                                (group[0].isTiled &&
                                    Twm.getTileGroupFor(group[0]).length <=
                                        1)) &&
                            winTracker.get_window_app(group[0]) ===
                                winTracker.get_window_app(w)
                        ) {
                            group.push(w);
                            return allGroups;
                        }
        ...
        _setIconSize() {
            ...
            let parentPadding = this.get_parent()
                .get_theme_node()
        ...
        vfunc_allocate(box) {
            ...
            let arrowHeight = Math.floor(
                this.get_theme_node().get_padding(St.Side.BOTTOM) / 3,
            );
            ...
            for (let i = 0; i < this._items.length; i++) {
                ...
                childBox.x1 =
                    contentBox.x1 +
                    Math.floor(
                        itemBox.x1 + (itemBox.x2 - itemBox.x1 - arrowWidth) / 2,
                    );
...
const AppSwitcherItem = GObject.registerClass(
    ...
    class AppSwitcherItem extends St.BoxLayout {
        _init(windows) {
            ...
            const tileGroup =
                windows[0].isTiled && Twm.getTileGroupFor(windows[0]);
            this.isTileGroup =
                tileGroup &&
                tileGroup.every((w) => windows.includes(w)) &&
                tileGroup?.length > 1;
            ...
            this.set_size = (size) =>
                this.appIcons.forEach((i) => i.set_size(size));
            this.label = null;
            this.app = {

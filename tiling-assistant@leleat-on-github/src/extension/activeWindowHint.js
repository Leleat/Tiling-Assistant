...
const Hint = GObject.registerClass(
    class ActiveWindowHint extends St.Widget {
        _init() {
            ...
            this._borderSize = Settings.getInt(
                'active-window-hint-border-size',
            );
            this._innerBorderSize = Settings.getInt(
                'active-window-hint-inner-border-size',
            ); // 'Inner border' to cover rounded corners
            this._settingsIds = [];

            this._settingsIds.push(
                Settings.changed('active-window-hint-color', () => {
                    this._color = Settings.getString(
                        'active-window-hint-color',
                    );
                }),
            );
            this._settingsIds.push(
                Settings.changed('active-window-hint-border-size', () => {
                    this._borderSize = Settings.getInt(
                        'active-window-hint-border-size',
                    );
                }),
            ...
            this._settingsIds.push(
                Settings.changed('active-window-hint-inner-border-size', () => {
                    this._innerBorderSize = Settings.getInt(
                        'active-window-hint-inner-border-size',
                    );
                }),
    ...
    class MinimalActiveWindowHint extends Hint {
        ...
        _onWsSwitched() {
            ...
            const notOverlappedWindowExists = windows.some((w) => {
                if (
                    !overlapping.some((o) =>
                        o.get_frame_rect().overlap(w.get_frame_rect()),
                    )
                ) {
                    return true;
                }
...
const AlwaysHint = GObject.registerClass(
    class AlwaysActiveWindowHint extends Hint {
        ...
        _updateGeometry() {
            ...
            this._showLater = global.compositor
                .get_laters()

		...
		open(openWindows, tiledWindow) {
			...
			openWindows.forEach(w => {
				if (w.get_id() != tiledWindow.get_id()) {
					...
					pos += (ICON_SIZE + 16 + ICON_MARGIN + ((SHOW_LABEL) ? 28 : 0)) * monitorScale; // magicNr are margins/paddings from the icon to the full-sized highlighted button
				}
			});
			...
			// setup bgGrid
			this.bgGrid.show();
			this.bgGrid.set_size(pos, (ICON_SIZE + 16 + ICON_MARGIN + ((SHOW_LABEL) ? 28 : 0)) * monitorScale);
